...
public class PlateLocate {
     ...
     */
    public Vector<MatAndPoint> plateLocate(Mat src) {
        //Vector<Mat> resultVec = new Vector<Mat>();
        
        Vector<MatAndPoint> vMatAndPoint = new Vector<MatAndPoint>();
            ...
            if (verifySizes(minRect)) {
                ...
                if (debug) {
                    ...
                    for (int j = 0; j < 4; j++) {
                        ...
                        AndroidOut.showImage("",contours.get(i));;
                    }
                }
                ...
                if (angle - this.angle < 0 && angle + this.angle > 0) {
                    ...
                    for (int j = 0; j < 4; j++) {
                        Point pt1 = new Point(rect_points.position(j).asCvPoint2D32f());
                        Point pt2 = new Point(rect_points.position((j + 1) % 4).asCvPoint2D32f());
                        line(resultMat, pt1, pt2, new Scalar(0, 255, 255, 255), 1, 8, 0);
                        if(j==1){
                        	jPoint.setX1(pt1.x());
                            jPoint.setY1(pt1.y());
                        }
                    ...
                    matAndPoint.setjPoint(jPoint);
					vMatAndPoint.add(matAndPoint);
                }
            }
        }
        if (debug) {
